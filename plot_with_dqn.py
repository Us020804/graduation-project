...
plt.plot(episodes, dqn_with_cost, linestyle=':', linewidth=3, label='DQN (With Move Cost, Eval)')

plt.xlabel('Episode')
plt.ylabel('Total Reward')
plt.title('Final Comparison: A*, Q-learning, DQN')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
print(f"DQN 有移动代价(评估): {sum(dqn_with_cost)/len(dqn_with_cost):.2f}")